...
export function calculateBSFI(
    ...
    // session gates penalties that are only meaningful on evening entries:
    //   — focus < 2 (SES, RDS): focus_hours = 0 on a morning entry because
    //     the day has not happened. Zero is structural, not a signal.
    //   — entropy_reset absent (SES): this is an evening tag. Its absence
    //     from a morning entry is guaranteed and carries no information.
    // On morning entries these penalties produce contradictory output —
    // inflating SES scores for people with neutral mood and low tension.
    ...
    // Focus + acoustic compound: cognitive capacity spent on noise filtering.
    // Valid for both sessions — daytime_db is logged in morning entries too.
    // The compound requires daytime_db > 55 as a guard, so null/low db is safe.
    if (safe(today.focus_hours) < 3 && today.daytime_db !== null && today.daytime_db > 55) {
        ses += 5;
    }

    // Focus standalone: attentional capacity constrained by environment.
    // EVENING ONLY — focus_hours = 0 on every morning entry because the day
    // has not yet happened. Van Dongen (2003) addresses sustained cognitive
    // load across a waking day, not pre-day zero. Applying this to morning
    // entries penalises structural null data, not a real signal.
    if (session === 'evening' && safe(today.focus_hours) < 2) ses += 3;

    // Entropy reset: absence of deliberate evening spatial reset.
    // EVENING ONLY — entropy_reset is defined as an evening tag. On a morning
    // entry evening_tags = [] always, so this would fire unconditionally and
    // add +3 to every morning score regardless of actual conditions.
    ...
    // EVENING ONLY — Van Dongen et al. (2003): sustained cognitive load
    // across the waking day increases allostatic load carried into sleep.
    // This is an end-of-day signal. On a morning entry focus_hours = 0
    // because the day has not happened — applying this penalty produces
    // contradictory scores (inflating RDS for people with low tension and
    // few wakes, as in the case that triggered this fix).
    if (session === 'evening') {
