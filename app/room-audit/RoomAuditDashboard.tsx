...
import { Camera, Loader2, ScanEye, CheckCircle, Brain, Lightbulb, Zap, Info } from 'lucide-react'
...
interface RoomAuditDashboardProps {
  ...
  result?: any
  loading?: boolean
  errorMessage?: string | null // 🟢 Added new prop
}

export default function RoomAuditDashboard({ hasAccess, rooms, onRunAnalysis, result, loading, errorMessage }: RoomAuditDashboardProps) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      {/* ------------------- UPLOAD PANEL ------------------- */}
      <div className="glass-panel p-8 rounded-3xl border border-[#c9ccbb]/10 bg-[#000]/20 relative overflow-hidden flex flex-col">
          ...
          <div className="inline-flex bg-[#000]/40 rounded-full p-1 border border-[#c9ccbb]/10 flex-wrap justify-center gap-1">
            {rooms.map(room => (
              <button
                ...
                disabled={!hasAccess}
                className={`px-3 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest ${selectedRoom === room ? 'bg-[#b5a642] text-[#1b270e]' : 'text-[#c9ccbb]/40 hover:text-[#c9ccbb]'}`}
              >
                {room}
              </button>
            ))}
        ...
        <div onClick={() => hasAccess && fileInputRef.current?.click()}
             className={`w-full flex-1 min-h-[200px] rounded-2xl border-2 border-dashed flex flex-col items-center justify-center relative cursor-pointer
        ...
        <div className="mt-6 flex flex-col items-center">
          ...
          >
            {loading ? <><Loader2 size={14} className="animate-spin" /> Analysing</> : <>Run Diagnosis <Brain size={14} /></>}
          </button>
          
          {/* 🟢 THE GRACEFUL ERROR DISPLAY */}
          {errorMessage && (
            <div className="mt-4 text-red-400 text-xs font-medium bg-red-900/20 px-4 py-2 rounded-lg border border-red-900/50">
              {errorMessage}
            </div>
          )}
        </div>
      ...
      {/* ------------------- RESULTS PANEL (Remains Unchanged) ------------------- */}
      <div className="relative h-full min-h-[400px]">
        {/* ... (Keep your exact existing result mapping here) ... */}
        {result ? (
          <div className="glass-panel p-8 rounded-3xl border border-[#b5a642]/30 bg-[#1b270e] h-full">
