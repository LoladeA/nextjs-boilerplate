...
export default function RoomAudit({ isPremium = false }: Props) {
  ...
  const [godMode, setGodMode] = useState(false) // Special bypass for founder/test user
  ...
  // --------------------------------------------------
  // 1️⃣ CHECK PREMIUM STATUS BEFORE ANY INTERACTION
  // --------------------------------------------------
  useEffect(() => {
  ...
  // --------------------------------------------------
  // 2️⃣ FILE HANDLING (DISABLED IF NO ACCESS)
  // --------------------------------------------------
  ...
  if (!hasAccess) {
    return (
      <div className="min-h-screen bg-[#1b270e] font-sans flex flex-col items-center justify-center text-center p-12">
        ...
        <Link
          href="/upgrade"
          className="px-8 py-3 bg-[#b5a642] text-[#1b270e] rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-[#d4c55e]"
        >
          Upgrade to Premium
        </Link>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="glass-panel p-8 rounded-3xl border border-[#c9ccbb]/10 bg-[#000]/20 relative overflow-hidden h-fit">
              ...
              <div onClick={() => fileInputRef.current?.click()}
                className={`w-full aspect-square rounded-2xl border-2 border-dashed flex flex-col items-center justify-center relative overflow-hidden cursor-pointer transition-all ${previewUrl ? 'border-[#b5a642] bg-black' : 'border-[#c9ccbb]/20 hover:border-[#b5a642]/50'}`}>
                <input type="file" ref={fileInputRef} onChange={handleFileSelect} accept="image/*" className="hidden" />
                {previewUrl ? <img src={previewUrl} alt="Preview" className="absolute inset-0 w-full h-full object-cover opacity-60" /> : (
