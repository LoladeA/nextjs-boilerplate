...
// =============================================================================
// UPDATE NUDGE BANNER — React Component
// =============================================================================
//
// Usage in dashboard:
//
//   const nudge = shouldShowNudge(user.baseline_date, user.last_update_date)
//   <UpdateNudgeBanner nudge={nudge} onStart={() => router.push('/assessment/update')} />
//
// Dismisses per session via React state — not localStorage (not supported).
...
interface UpdateNudgeBannerProps {
  nudge:   NudgeConfig
  onStart: () => void
}

export const UpdateNudgeBanner = ({ nudge, onStart }: UpdateNudgeBannerProps) => {
  ...
  return (
    <div className={`
      relative w-full rounded-2xl border p-5 mb-6 flex items-start justify-between gap-4
      transition-all duration-300
      ${isPresent
        ? 'bg-[#b5a642]/10 border-[#b5a642]/30'
        : 'bg-[#1b270e]/5 border-[#1b270e]/15'
      }
    `}>
      {/* Gold accent bar */}
      <div className="absolute left-0 top-4 bottom-4 w-[3px] bg-[#b5a642]/60 rounded-full" />

      <div className="pl-4 flex-1">
        <p className={`text-sm font-semibold mb-1 ${isPresent ? 'text-[#b5a642]' : 'text-[#1b270e]/70'}`}>
          {nudge.label}
        </p>
        <p className="text-xs text-[#1b270e]/50 mb-4">
          {nudge.sublabel}
        </p>
        <button
          onClick={onStart}
          className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#1b270e] hover:text-[#b5a642] transition-colors"
        >
          Begin Check-In <ArrowRight size={12} />
        </button>
      </div>
      ...
      <button
        ...
        className="text-[#1b270e]/25 hover:text-[#1b270e]/50 transition-colors mt-0.5 shrink-0"
        aria-label="Dismiss"
      >
        <X size={14} />
      </button>
