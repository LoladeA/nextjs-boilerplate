...
import { ArrowRight, Layout, Eye, Shield, Sparkles, CheckCircle } from 'lucide-react'
...
export default function Week3Nudge() {
  ...
  // 🟢 FIXED: Added the data fetch logic to track progress for this specific module ID
  useEffect(() => {
    const fetchScore = async () => {
      const { data } = await supabase
        .from('quiz_submissions')
        .select('score')
        .eq('module_id', 'sensory-lighting-dynamics-week-3') 
        ...
        <div className="max-w-4xl mx-auto text-[#c9ccbb]">
          ...
          <header className="mb-20 text-center animate-fade-in-up">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#b5a642]/10 text-[#b5a642] text-xs font-bold uppercase tracking-widest mb-6">
              <CheckCircle size={14} /> Week 3 Complete
            </div>
            <h1 className="text-4xl md:text-5xl font-serif mb-6 leading-tight">Integration Strategy</h1>
          ...
          <div className="grid gap-12 mb-20">
            {/* 1. Visual Load */}
            <div className="glass-panel p-10 rounded-[2rem] border border-[#b5a642]/20 bg-[#b5a642]/5 shadow-xl transition-all hover:border-[#b5a642]/40">
              ...
              <p className="text-lg leading-relaxed opacity-80 mb-6">
                <strong>High Visual Load:</strong> Transition to matte surfaces in your primary workspace. Reducing specular reflection minimizes microsaccades, easing cortical strain and preserving working memory for deep-focus tasks.
              </p>
            </div>

            {/* 2. Vigilance Mitigation */}
            <div className="glass-panel p-10 rounded-[2rem] border border-[#b5a642]/20 bg-[#b5a642]/5 shadow-xl transition-all hover:border-[#b5a642]/40">
              <div className="flex items-center gap-6 mb-8">
                <div className="w-14 h-14 bg-[#b5a642] text-[#1b270e] rounded-full flex items-center justify-center">
                  <Shield size={28} />
              ...
              <div className="space-y-6">
                <p className="text-lg leading-relaxed opacity-80 border-l-2 border-[#b5a642]/30 pl-4">
                  <strong>High Amygdala Reactivity:</strong> Prioritize indirect lighting and wall-wash techniques. Diffuse illumination reduces high-contrast shadows, signaling environmental safety to the threat-detection centers of the brain.
                </p>
                <p className="text-lg leading-relaxed opacity-80 border-l-2 border-[#b5a642]/30 pl-4">
                  <strong>Predictive Legibility:</strong> Use hierarchical accents to clarify focal points. Neuroscience confirms that clear spatial anchors enhance predictive coding, significantly minimizing cognitive friction during room navigation.
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Upsell Section */}
          <div className="p-12 rounded-[2rem] border border-[#b5a642]/40 bg-[#b5a642]/10 mb-20 text-center relative overflow-hidden group">
            <Sparkles className="absolute top-4 right-4 text-[#b5a642]/40 group-hover:scale-110 transition-transform" size={32} />
