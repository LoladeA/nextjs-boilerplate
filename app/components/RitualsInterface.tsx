...
const HIGH_LOAD_THRESHOLD = 65

// Maps time of day to the two ritual IDs available for that window
const RITUAL_ID_MAP: Record<string, { normal: string; high: string }> = {
  morning:   { normal: 'morning-activation', high: 'morning-calm'    },
  afternoon: { normal: 'afternoon-focus',    high: 'afternoon-reset'  },
  evening:   { normal: 'evening-taper',      high: 'evening-shelter'  }
}
...
export default function RitualsInterface({
  neuroLoadScore,
  profile            = 'anchor',
  ...
  // Resolve the correct nine-profile variant using the exported helper.
      ...
      >
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border ${
                  isOpen
                    ? 'text-[#b5a642] border-[#b5a642]/30 bg-[#b5a642]/5'
                    : 'text-[#c9ccbb]/60 border-[#c9ccbb]/20'
                }`}>
                  {timeOfDay} Ritual
                </span>
                {isHighLoad && (
        ...
        <AnimatePresence>
          {isOpen && (
            ...
            >
              <div
                className="px-6 md:px-8 pb-8 pt-0 relative z-10 border-t border-[#b5a642]/10 mt-2 cursor-default"
                    ...
                    return (
                      <div
                        ...
                        className={`
                          ...
                          ${isChecked
                            ? 'bg-[#1b270e] border-[#b5a642]/10 opacity-60 hover:opacity-100'
                            : 'bg-[#c9ccbb]/5 border-[#b5a642]/10 hover:border-[#b5a642]/40 hover:bg-[#c9ccbb]/10'
                          }
                        `}
                      >
                        <div className={`
                        ...
                        <div className="space-y-1 w-full">
                          <div className="flex justify-between items-start">
                            <h4 className={`font-bold text-sm uppercase tracking-wide transition-colors ${
                              isChecked ? 'text-[#c9ccbb]/40 line-through decoration-[#b5a642]/50' : 'text-[#c9ccbb]'
                            }`}>
                              {step.label}
                            </h4>
                            <div className="flex gap-2">
                              {step.duration && (
                                <span className={`flex items-center gap-1 text-[10px] uppercase font-bold px-2 py-0.5 rounded border transition-colors ${
                                  isChecked
                                    ? 'text-[#c9ccbb]/30 border-[#c9ccbb]/10'
                                    : 'text-[#b5a642] bg-[#b5a642]/10 border-[#b5a642]/20'
                                }`}>
                              ...
                              {step.toolLink && (
                                <Link href={step.toolLink} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}>
                                  <span className={`flex items-center gap-1 text-[10px] uppercase font-bold border px-2 py-0.5 rounded transition-colors cursor-pointer ${
                                    isChecked
                                      ? 'text-[#c9ccbb]/30 border-[#c9ccbb]/10'
                                      : 'text-[#c9ccbb]/60 hover:text-[#b5a642] border-[#c9ccbb]/20 hover:border-[#b5a642]'
                                  }`}>
                          ...
                          <p className={`text-sm leading-relaxed transition-colors ${
                            isChecked ? 'text-[#c9ccbb]/30' : 'text-[#c9ccbb]/70'
                          }`}>
                            {step.instruction}
                          </p>
                        </div>
