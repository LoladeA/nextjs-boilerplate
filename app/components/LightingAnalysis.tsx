...
import { Sun, Moon, Eye, Activity, Zap } from 'lucide-react'

// This component accepts the raw scores and determines which science module to show
export default function LightingAnalysis({ scores }: { scores: any }) {
  
  // LOGIC: Triggers warnings if score is below 60 (Adjust threshold as needed)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-4 pt-8 border-t border-[#c9ccbb]/10">
        <Sun className="text-[#b5a642]" size={28} />
        <h2 className="text-2xl font-serif text-[#c9ccbb]">Photobiology & Circadian Health</h2>
      </div>
      
      <p className="text-[#c9ccbb]/60 mb-8 max-w-2xl">
        Light is not just illumination—it entrains your internal clock and regulates hormones. 
        Here is how your current lighting environment is impacting your nervous system.
      </p>

      {/* MODULE 1: CIRCADIAN ALIGNMENT */}
      <div className={`glass-panel p-8 rounded-2xl border-l-4 ${showCircadian ? 'border-red-400' : 'border-[#b5a642]'}`}>
        <div className="flex justify-between items-start mb-4">
          ...
          <span className={`px-3 py-1 text-xs rounded-full uppercase tracking-widest ${showCircadian ? 'bg-red-400/20 text-red-400' : 'bg-[#b5a642]/20 text-[#b5a642]'}`}>
            {showCircadian ? 'Misalignment Detected' : 'Optimized'}
          </span>
        </div>
        ...
        <p className="text-[#c9ccbb]/80 mb-6 leading-relaxed">
          ...
          {showCircadian 
            ? " Your input suggests misaligned lighting (too bright at night or too dim in morning), which suppresses melatonin and fragments sleep."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-[#1b270e]/30 p-4 rounded-xl">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <span className="text-[#c9ccbb]/60 text-sm">Target 4500–6500K light (blue-enriched) before 10 AM.</span>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div>
                ...
                <span className="text-[#c9ccbb]/60 text-sm">Switch to {`<3000K`} warm, dim lighting 2 hours before bed.</span>
              </div>
            </div>
          </div>
        )}
      ...
      {showCortisol && (
        <div className="glass-panel p-8 rounded-2xl border-l-4 border-orange-400">
          ...
          <div className="bg-[#1b270e]/30 p-4 rounded-xl border border-orange-400/20">
             <h4 className="text-orange-400 text-xs font-bold uppercase mb-2">Prescription</h4>
             <ul className="text-sm text-[#c9ccbb]/70 space-y-2 list-disc pl-4">
               ...
               <li><strong>Wavelength Adjustment:</strong> Use "phase-appropriate" warm profiles in the evening.</li>
             </ul>
          </div>
        </div>
      )}

      {/* MODULE 3: VISUAL GEOMETRY */}
        ...
        {showGeometry && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             <div className="p-4 rounded-lg bg-[#c9ccbb]/5 border border-[#c9ccbb]/10">
               <span className="block text-[#b5a642] font-bold text-sm mb-1">Layering</span>
               <span className="text-xs text-[#c9ccbb]/60">Mix indirect ambient light with focused task lighting.</span>
             </div>
             <div className="p-4 rounded-lg bg-[#c9ccbb]/5 border border-[#c9ccbb]/10">
               <span className="block text-[#b5a642] font-bold text-sm mb-1">Diffusion</span>
               <span className="text-xs text-[#c9ccbb]/60">Use shades or bounce light off walls to soften harsh contrasts.</span>
             </div>
             <div className="p-4 rounded-lg bg-[#c9ccbb]/5 border border-[#c9ccbb]/10">
               <span className="block text-[#b5a642] font-bold text-sm mb-1">Flicker</span>
