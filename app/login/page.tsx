...
export default function LoginPage() {
  ...
  // This function now handles 'github' or 'google'
  const handleOAuth = async (provider: 'github' | 'google') => {
    setLoading(true)
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        // Critical: This must match what is allowed in Supabase -> Authentication -> URL Configuration
    ...
    <div className="min-h-screen bg-[#1b270e] flex flex-col items-center justify-center px-6">
      ...
      <div className="w-full max-w-md bg-[#c9ccbb] rounded-[2rem] p-10 md:p-14 shadow-2xl">
        ...
        <div className="space-y-3 mb-8">
          {/* GitHub Button - Matched to your settings */}
          <button
          ...
          {/* Google Button - Kept as option if needed */}
          <button
            onClick={() => handleOAuth('google')}
            className="w-full py-4 bg-white border border-[#1b270e]/10 text-[#1b270e] rounded-full font-medium hover:bg-[#f0f0f0] transition-all flex items-center justify-center gap-3"
          >
